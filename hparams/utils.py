...
def _reuse_fail(name, values):
  ...
  raise ValueError(
      'Multiple assignments to variable \'%s\' in %s' % (name, values))
...
def parse_values(values, type_map):
  ...
  functions.  (And must be first explicit added through setattr.)
    ...
    for name, value in hparam_def.hparam.items():
      ...
      if kind.endswith('_value'):
        ...
        if kind.startswith('int64'):
          ...
          setattr(self, name, int(getattr(value, kind)))
          ...
          setattr(self, name, compat.as_str(getattr(value, kind)))
        else:
          setattr(self, name, getattr(value, kind))
      else:
        # List of values.
        if kind.startswith('int64'):
          # Setting attribute value to be 'int' to ensure the type is compatible
          # with both Python2 and Python3.
          setattr(self, name, [int(v) for v in getattr(value, kind).value])
        elif kind.startswith('bytes'):
          # Setting attribute value to be 'str' to ensure the type is compatible
          # with both Python2 and Python3. UTF-8 encoding is assumed.
          setattr(self, name,
          ...
          setattr(self, name, [v for v in getattr(value, kind).value])
